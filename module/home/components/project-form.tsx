...
import { onInvoke } from "@/module/home/actions";
...
const pollBuildStatus = async (): Promise<{
    status: string
    sandboxUrl?: string
    error?: string
}> => {
    ...
    for (let i = 0; i < maxAttempts; i++) {
        ...
        if (data.status === "completed" && data.sandboxUrl) {
            return { status: "completed", sandboxUrl: data.sandboxUrl }
        }
        if (data.status === "failed") {
            return { status: "failed", error: data.error ?? "Build failed" }
        }
        await new Promise((r) => setTimeout(r, 2000))
    }
    return { status: "timeout", error: "Build timed out. Check http://localhost:8288" }
}
...
const ProjectForm = () => {
    ...
    const onSubmit = async (values: z.infer<typeof formSchema>) => {
        try {
            setIsPending(true)
            setPreviewUrl(null)
            await onInvoke(values.content)
            toast.message("Building your app…", {
                description: "This usually takes 1–3 minutes.",
            })

            const build = await pollBuildStatus()

            if (build.status === "completed" && build.sandboxUrl) {
                setPreviewUrl(build.sandboxUrl)
                toast.success("Preview ready!")
                form.reset()
            } else {
                toast.error(build.error ?? "Build failed")
            }
        } catch (error) {
            console.error(error)
    ...
    return (
        <div className="space-y-8">
            {previewUrl && (
                <div className="rounded-xl border bg-card p-4 space-y-3">
                    ...
                    <iframe
                        title="App preview"
                        src={previewUrl}
                        className="w-full h-[min(70vh,640px)] rounded-lg border bg-background"
                    />
                </div>
            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                {Project_Template.map((template, index) => (
                    <button
                        key={index}
